import { AlertCircle, CheckCircle, InfoCircle } from "@a-peak-works/untitledui-icons";
...
export const AlertFloating = ({ title, description, confirmLabel, onClose, onConfirm, color = "default" }: AlertProps) => {
    return (
        <div className="relative flex flex-col gap-4 rounded-xl border border-primary bg-primary_alt p-4 shadow-xs md:flex-row">
            <FeaturedIconBase icon={iconMap[color]} color={color === "default" ? "gray" : color} theme={color === "default" ? "modern" : "outline"} size="md" />

            <div className="flex flex-1 flex-col gap-3 md:w-0">
                <div className="flex flex-col gap-1 overflow-auto">
                    <p className="pr-8 text-secondary tt-sm-semi md:truncate md:pr-0">{title}</p>
...
AlertFloating.parameters = {
    design: {
        urls: {
            desktop: "https://www.figma.com/design/BXDc9uTbY80aquiwCK1FYs?node-id=4050-412599",
            mobile: "https://www.figma.com/design/BXDc9uTbY80aquiwCK1FYs?node-id=4050-412610",
        },
    },
};
            ...
            <div className="mx-auto flex max-w-container flex-col gap-4 p-4 md:flex-row md:items-center md:gap-3 md:px-8 md:py-3">
                ...
                <div className="flex gap-2">
                    <div className={cx("flex w-full gap-3", actionType === "button" ? "flex-col-reverse md:flex-row" : "flex-row")}>
                        <Button onClick={onClose} color={actionType === "button" ? "secondary-gray" : "link-gray"} size={actionType === "button" ? "md" : "sm"}>
                            Dismiss
                        </Button>
...
AlertFullWidth.parameters = {
    design: {
        urls: {
            desktop: "https://www.figma.com/design/BXDc9uTbY80aquiwCK1FYs?node-id=4893-410873",
            mobile: "https://www.figma.com/design/BXDc9uTbY80aquiwCK1FYs?node-id=4893-411092",
        },
    },
};
